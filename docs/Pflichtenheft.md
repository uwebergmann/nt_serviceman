...
**Hinweis zu netbox_manufacturer, netbox_model, netbox_firmware_version:**  
Diese drei Felder werden in `_extract_netbox_fields()` aus dem NetBox-Device-JSON übernommen. Sie sind Voraussetzung für die CPE-String-Erzeugung (CVE-Monitoring, nt_cve-automatisation). Die Firmware-Version kommt aus `custom_fields`; **fehlt sie in NetBox, bleibt das Feld leer – das Programm wird nicht behindert.** CPE kann auch ohne Version gebaut werden (z.B. mit `*`).
...
- REST-API-Anbindung, Felderübernahme (Name, Rolle, Tenant, Serial, Hardware-Typ, Hersteller, Modell, Firmware-Version, …)
